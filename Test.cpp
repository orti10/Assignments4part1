...
testcase.setname("Testing smart guesser");
	...
	SmartGuesser smart;
		...
		for (uint i=0; i<20; ++i) {
			testcase.CHECK_EQUAL(play(rand, smart, 4, 100)<30, true);  // smarty should always win in at most 30 turns!
		}
		for (uint i=0; i<20; ++i) {
			testcase.CHECK_EQUAL(play(rand, smart, 4, 100)<20, true);  // smarty should always win in at most 20 turns!
		}
		for (uint i=0; i<50; ++i) {
			testcase.CHECK_EQUAL(play(rand, smart, 4, 100)<15, true);  // smarty should always win in at most 15 turns!
		}
		for (uint i=0; i<100; ++i) {
			testcase.CHECK_EQUAL(play(rand, smart, 4, 100)<=10, true);  // smarty should always win in at most 10 turns!
		}
		/* ---------------------End of checking input in length 4 ---------------------*/
		...
		.CHECK_EQUAL(play(c4567, smart, 4, 100)<=10, true)
		.CHECK_EQUAL(play(c1089, smart, 4, 100)<=10, true)
		.CHECK_EQUAL(play(c4632, smart, 4, 100)<=10, true)
		.CHECK_EQUAL(play(c1234, smart, 4, 100)<=10, true)
		.CHECK_EQUAL(play(c12345, smart, 5, 100)<=100, true)
		.CHECK_EQUAL(play(c9999, smart, 4, 100)<=10, true)

		/* Checking for the spacial cases */
		.CHECK_EQUAL(play(c0000, smart, 4, 100)<=10, true) // checking "0000"
		.CHECK_EQUAL(play(c0001, smart, 4, 100)<=10, true) // // checking "0001"
	;
